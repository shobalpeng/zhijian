...
export default function EditWanderPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!location.trim() || !date) { toast.error("请填写地点和日期"); return; }
    setSubmitting(true);
    try {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/wanders/${id}`, { method: "DELETE" });
  ...
  if (loading) {
    return (
      <>
        <TopBar title="编辑记录" showBell={false} />
    ...
    <div className="flex flex-col min-h-full">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 py-6 flex-1">
        ...
        <div className="mt-auto pt-4 space-y-3">
          ...
          <Button type="button" variant="destructive" className="w-full" onClick={handleDelete}>删除记录</Button>
        </div>
      </form>
    </div>
  );

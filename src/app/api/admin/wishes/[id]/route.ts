...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (fulfillerId !== undefined) updates.fulfillerId = Number(fulfillerId);
  db.update(wishes).set(updates).where(eq(wishes.id, Number(id))).run();
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session.userId || !session.isAdmin) return Response.json({ error: "Forbidden" }, { status: 403 });
  const { id } = await params;
  db.delete(wishes).where(eq(wishes.id, Number(id))).run();
  return Response.json({ ok: true });
}

...
export default function CreateTravelPage() {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <TopBar title="添加目的地" showBell={false} />
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 py-6 flex-1">
          ...
          {coverImage ? (
            <div className="relative rounded-lg overflow-hidden ring-1 ring-foreground/10">
              ...
              <button
                ...
                className="absolute top-2 right-2 rounded-full bg-background/80 p-1.5 text-xs text-muted-foreground"
              >
                移除
              </button>
            </div>
        ...
        {(lat || lng) && (
          <div className="space-y-2">
            <Label>经纬度（已定位）</Label>
            <div className="flex gap-2">
              <Input type="number" step="any" value={lat} onChange={(e) => setLat(e.target.value)} placeholder="纬度" className="flex-1" />
              <Input type="number" step="any" value={lng} onChange={(e) => setLng(e.target.value)} placeholder="经度" className="flex-1" />
            </div>
          </div>
        )}
